...
**Pushes and restarts:** Pushing to **GitHub alone** does **not** update the Space unless the Space is wired to that repo. Pushing to the Space’s **`hf` git remote** triggers a **new Docker build**; the old container keeps running until the build **finishes** (often **many minutes**). **`GET /v1/health`** should show **`decimate_logic_version`: `"multipass-v2"`** and **`fast_simplification`: `true`** after a successful deploy. If **`fast_simplification`** is **`false`**, quadric decimation cannot run — check **Build logs** and `integrations/space_api/requirements.txt`. Use **Factory rebuild** only when you need a clean image; you still need the **latest commit** on the Space.
