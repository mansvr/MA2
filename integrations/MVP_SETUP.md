...
**Pushes and restarts:** Many Spaces are **wired to GitHub**, not to the copy you push with **`git push hf`**. If **`git log ma2/main`** (GitHub) is behind **`git log hf/main`** (Hub), the Space will keep building **old code** until you **`git push ma2 main`** (or merge) so GitHub matches. Pushing to **GitHub alone** does **not** update a Hub-only remote, and **`hf` alone** does **not** update GitHub. Pushing to the Space’s **`hf` git remote** triggers a **new Docker build** when the Space tracks that repo; the old container keeps running until the build **finishes** (often **many minutes**). **`GET /v1/health`** should show **`decimate_logic_version`** (e.g. `"multipass-v4"`) matching **`DECIMATE_LOGIC_VERSION`** in `integrations/space_api/trimesh_decimate.py` **on the commit HF built** — if health stays on an old string, you likely **did not commit and push** that change, or the build has not finished. **`fast_simplification`: `true`** should hold after a successful deploy. If **`fast_simplification`** is **`false`**, quadric decimation cannot run — check **Build logs** and `integrations/space_api/requirements.txt`. **Factory rebuild** only clears Docker layer caches; it still builds whatever commit is on the Hub — it does not pick up **uncommitted** local edits.
